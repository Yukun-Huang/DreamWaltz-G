...
def get_backend():
    ...
    if BACKEND is None:
        try:
            import sys
            # sys.path.append('/home/huangyukun/.cache/torch_extensions/py38_cu116/ours/_raymarchinglatent/')
            # sys.path.append('/comp_robot/zhengtianze/dreamwaltz/dreamwaltz-gaussian/core/nerf/raymarching/latent/build/')
            import os.path as osp
            sys.path.append(osp.join(osp.dirname(osp.abspath(__file__)), 'build'))
            import _raymarchinglatent as _backend
        except ImportError:
            # assert 0, 'Please build the extension "_raymarchinglatent" locally.'
            # print('Please build the extension "_raymarchinglatent" locally.')
